...
const AllSchedules = () => {
  ...
  useEffect(() => {
    const getAllSchedulesTwo = async ()=>{
      try {
        const somedata = await axios(`${process.env.NEXT_PUBLIC_SERVER_URL}/schedule/getAll`, {params:{counter, data:value !==0?value:null}});
  ...
  const LoadMore = async ()=> {
    ...
    try {
      ...
      const somedata = await axios(`${process.env.NEXT_PUBLIC_SERVER_URL}/schedule/getAll`, {params:{counter}});
  ...
  return (
    <main className={style.schedules}>
      <div>
        <button   onClick={()=>setClicked(true)} className={style.btn} >Поиск по дате</button>
      </div>
     {clicked &&  <div style={{display:'flex', justifyContent: 'center', margin: 10}} >
      ...
      <button className={style.btn} onClick={()=>LoadMore()} >Загрузить еще</button>
    </main>
  );
};
